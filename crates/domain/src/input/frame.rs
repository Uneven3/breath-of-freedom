...
impl ActiveActions {
    ...
    pub fn set_pressed(&mut self, source: InputSource, action: IntentAction, pressed: bool) {
    ...
    pub fn trigger(&mut self, source: InputSource, action: IntentAction) {
        if let Some(frame) = self.frame_mut(source) {
            frame.trigger(action);
        }
    }

    pub fn clear_pressed(&mut self, source: InputSource) {
        if let Some(frame) = self.frame_mut(source) {
            frame.held = 0;
        }
    }
    ...
    pub fn discard_pending(&mut self, source: InputSource) {
        if let Some(frame) = self.frame_mut(source) {
