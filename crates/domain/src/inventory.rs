...
impl Inventory {
    ...
    pub fn try_add(&mut self, kind: ItemKind, quantity: u32) -> Result<(), InventoryAddError> {
        if quantity == 0 {
            return Err(InventoryAddError::ZeroQuantity);
        }
            ...
            for slot in self.slots.iter_mut().flatten() {
                if slot.kind == kind {
                    let Some(total) = slot.quantity.checked_add(quantity) else {
                        return Err(InventoryAddError::QuantityOverflow);
                    };
                    slot.quantity = total;
                    return Ok(());
                }
...
mod tests {
    ...
    fn try_add_rejects_zero_quantity_and_forces_weapon_quantity_to_one() {
        ...
        assert_eq!(
            ...
            Err(InventoryAddError::ZeroQuantity),
    ...
    fn try_add_rejects_quantity_overflow_without_mutating_the_stack() {
        ...
        assert_eq!(
            inventory.try_add(ItemKind::Material(MaterialKind::Wood), 1),
            Err(InventoryAddError::QuantityOverflow)
        );
