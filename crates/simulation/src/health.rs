//! Authoritative hit-point mutation and death publication.

use bevy_app::{App, FixedUpdate, Plugin};
use bevy_ecs::{name::Name, prelude::*};
use bevy_log::info;

pub use bof_domain::health::{
...
pub enum HealthSet {
    /// Requests become pool mutations and death messages.
    Apply,
}
...
/// The only writer of [`Health`]. Missing and already-dead pools are ignored.
pub fn apply_damage(
...
mod tests {
    ...
    fn deaths(world: &World) -> Vec<Entity> {
        ...
        cursor.read(messages).map(|death| death.entity).collect()
    }
