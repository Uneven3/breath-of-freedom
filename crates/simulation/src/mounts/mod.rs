...
pub use data::{HorseSpawnRequest, MountTransitionRequest};
...
impl Plugin for MountsPlugin {
    fn build(&self, app: &mut App) {
        ...
        // Which scenes get a horse is composition, so the app writes
        // `HorseSpawnRequest` on entering one that declares it — the same
        // request the debug hub writes, so there is one spawn path, not two.
...
mod plugin_tests {
    ...
    fn real_app(order: PluginOrder) -> App {
        ...
        // No `ButtonInput` here on purpose: nothing in this crate reads
        // hardware, so the raw keyboard resource this used to seed had no
        // reader left. Intents arrive through `ActiveActions` (C2).
