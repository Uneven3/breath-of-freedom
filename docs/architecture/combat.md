...
|---|---|---|
| Sigilo: bonus de daño contra un enemigo **no alertado** (flechas y sneakstrike) | Combate lee Enemies (y Movement) | Query read-only sobre `enemies::Awareness` del objetivo: `is_alerted()` = full threat, sin bonus posible — la conciencia del enemigo manda, no el ángulo del golpe (por la espalda el cono de visión nunca llenó el medidor). El sneakstrike cuerpo a cuerpo exige además atacante en `Movement::LocomotionState == Sneak`; la flecha solo exige objetivo no alertado. El monto final se emite como `health::DamageRequestMessage` (`health.md`). Diseño fijado en ticket `enemy-awareness` (2026-07-15) |
