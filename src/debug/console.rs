...
//!
//! - **Periodic** — the perf section on a fixed cadence. This is the A/B time
//!   series: several samples per configuration, so a value can be averaged
//!   and settling after a knob change is visible rather than guessed.
...
pub(super) fn log_periodic(
    ...
    *next_at = now + PERIOD;
    ...
    let Some(line) = snapshot.line(SectionId::Perf) else {
        return;
    };
    info!("[debug] {line}");
}
