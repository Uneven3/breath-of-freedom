...
use crate::movement::intents::GaitIntent;
...
/// promoted to a component like every other per-actor motor latch. Cleared by
/// the dispatcher whenever the actor is not in Stairs.
...
pub(super) fn tick_body(row: &mut MotorTickItem, mas: &MoveAndSlide, time: &Time) {
    let Some(movement) = row.ground_movement else {
        return;
    };
    let Some(cached) = row.stairs_local.as_mut() else {
        return;
    };
    let dt = time.delta_secs();

    if row.stairs.on_stairs {
        cached.0 = Some(row.stairs.clone());
    }
    let effective_stairs = cached.0.as_ref().unwrap_or(row.stairs);
    ...
    let is_crouched = row.crouched.is_some_and(|c| c.0);
    ...
    let sprinting = row.intents.gait == GaitIntent::Sprint && row.stamina.current() > 0.0;
    ...
    let ascending_snap = slope_input > ASCEND_THRESHOLD && feet_gap > 0.0 && feet_gap <= max_snap;
    let descending_snap =
